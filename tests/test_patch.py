...
class TestResultAiCm(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for module_obj in SUPPORTED_MODULES_TO_PATCH:
            cls.original_funcs[(module_obj.module_name_to_patch, module_obj.func_name_to_patch)] = get_func(module_obj.module_name_to_patch, module_obj.func_name_to_patch)

    @mock.patch('result_ai_sdk.patch.add_to_queue')
    def test_openai_monitoring(self, mock_add_to_queue):
        for module_obj in SUPPORTED_MODULES_TO_PATCH:
            with self.subTest(module_name=module_obj.module_name_to_patch, func_name=module_obj.func_name_to_patch):
                mock_create = mock.create_autospec(self.original_funcs[(module_obj.module_name_to_patch, module_obj.func_name_to_patch)])
                mock_create.return_value = self.mock_create_response

                _patch_func(module_obj.module_name_to_patch, module_obj.func_name_to_patch, mock_create)
        
                # Create a context manager
                with result_ai("test_task", param="value"):
                    # The function should be wrapped
                    self.assertIsInstance(get_func(module_obj.module_name_to_patch, module_obj.func_name_to_patch), wrapt.FunctionWrapper)
                ...
                expected_data = {'user_id': '1897ce6d-5694-41ce-a75d-8ea9e4dc81b4',
                                 'task_name': 'test_task', 
                                 'user_input_args': {'param': 'value'}, 
                                 'request_data': {'llm_call_arguments': 
        ...
        for module_obj in SUPPORTED_MODULES_TO_PATCH:
            with self.subTest(module_name=module_obj.module_name_to_patch, func_name=module_obj.func_name_to_patch):
                ...
                with result_ai("test_task"):
                    ...
                    self.assertTrue("result_ai_wrapper" in current_func._self_wrapper.__name__)
